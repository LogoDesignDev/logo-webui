        ...
        <el-table-column width="150">
          <template slot-scope="scope">
            <el-button
              ...
              type="info" size="mini" round plain
              @click="handleUnfollowClicked(scope.$index, scope.row.uid)">
              取消关注
            </el-button>
            <el-button
              v-else type="primary"
...
<script>
import { getFollowList, follow, unfollow } from 'api/personal'
export default {
  data () {
    return {
  ...
  methods: {
      ...
      getFollowList().then((res) => {
        ...
        switch (data.code) {
          case 20000: // 获取成功
            ...
            this.followList.forEach((obj) => {
              obj.hasFollowed = true
            })
     ...
     * 取消关注
     */
    handleUnfollowClicked (index, uid) {
      const params = {
        uid: uid
      }
      unfollow(params).then((res) => {
        ...
        switch (data.code) {
          case 20000: // 修改成功
            ...
            this.showList[index].hasFollowed = false
            this.showList = this.showList.splice(0)
            break
        }
      })
    }
